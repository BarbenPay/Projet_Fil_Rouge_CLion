    ...
    for(int i=0,i<3,i++){
        for(int j=0,j<hauteur,j++){
            for(int k=0,k<largeur,k++){
                val_pixel = matriceimage[k][j][i];
                matriceimage[k][j][i] = val_pixel / seuil;
            }
        }
    }
}
...
void nature_objet(objet *test,int image_bin [1920][1080][3]){
    ...
    if(test->coor_D - test->coor_G + delta > test->coor_B - test->coor_H && test->coor_D - test->coor_G - delta < test->coor_B - test->coor_H){
        ...
        if(test->couleur == 'R'){
            ...
            seuil_couleur[1] = 0;
            seuil_couleur[2] = 0;
            if(image_bin[test->coor_G + (test->coor_D - test->coor_G) / pourcentage ][test->coor_H + (test->coor_B - test->coor_H) / pourcentage ][0] == seuil_couleur[0] && image_bin[test->coor_G + (test->coor_D - test->coor_G) / pourcentage ][test->coor_H + (test->coor_B - test->coor_H) / pourcentage ][1] == seuil_couleur[1] && image_bin[test->coor_G + (test->coor_D - test->coor_G) / pourcentage ][test->coor_H + (test->coor_B - test->coor_H) / pourcentage ][2] == seuil_couleur[2]){
                test->nature = 'C';
            }
            else{
                test->nature = 'S';
    ...
    *cpt = 0;
    ...
    for (int i = 0; i < hauteur; ++i) {
        for (int j = 0; j < largeur; ++j) {
            ...
            g = image[i][j][1];
            b = image[i][j][2];
            color = detecter_couleur(r,g,b);
            if(*cpt == 0 && color != 'N'){
                tab[cpt].couleur = color;
                tab[cpt].coor_H = i;
                tab[cpt].coor_B = i;
                tab[cpt].coor_D = j;
                tab[cpt].coor_G = j;
                propagation(tab[cpt],image);
                *cpt ++;
            }
            else if(*cpt != 0 && color != 'N'){
                for(int t=0,t<*cpt,t++){
...
void encadrement(image[1920][1080][3], int hauteur, int largeur,objet *test){
    for(int i=test->coor_G,i<test->coor_D+1,i++){
    ...
    for(int j=test->coor_H,j<test->coor_B+1,j++){
...
void sortie_image(image[1920][1080][3],int hauteur,int largeur,int dimmention){
    printf("%d",largeur);
    printf("%d",hauteur);
    printf("%d \n",dimmention);
    for(int k=0,k<3,k++){
